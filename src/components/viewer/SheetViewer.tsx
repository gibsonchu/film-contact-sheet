    ...
    <div className="min-h-dvh bg-noir">
      <header className="flex flex-wrap items-center gap-3 border-b border-white/8 px-4 py-3">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="truncate text-lg tracking-tight text-warm">{doc.sheet.title}</h1>
        </div>
        {mode === "preview" ? (
      ...
      <main id="main" className="mx-auto max-w-[1500px] px-4 py-6">
        <div className="texture-noise mx-auto w-full max-w-[1200px] shadow-[0_30px_90px_rgba(0,0,0,0.7)]">
          <SheetSvg
            doc={doc}
            layout={layout}
            urls={urls}
        ...
        {doc.sheet.description ? (
          <p className="mx-auto mt-6 max-w-2xl text-center text-[14px] leading-relaxed text-bone">
            {doc.sheet.description}
          </p>
        ) : null}
        ...
        <section className="mx-auto mt-8 grid max-w-4xl gap-6 md:grid-cols-2">
          <div>
            <h2 className="label mb-2">Frames</h2>
                ...
                .map((p) => (
                  <li key={p.id} className="flex gap-3 text-[13px]">
                    ...
                    <span className="text-bone">
                      ...
                      {p.publicNote ? (
                        <span className="mt-0.5 block text-[12px] text-smoke">{p.publicNote}</span>
                      ) : null}
                    </span>
                  </li>
                ))}
  ...
  return (
    <div>
      ...
      <ul className="mb-3 space-y-2">
        ...
        {comments.length === 0 ? <li className="text-[13px] text-smoke">No comments yet.</li> : null}
      </ul>
      <form
        className="space-y-2"
        onSubmit={async (e) => {
      ...
      >
        <p className="text-[11px] text-smoke">Commenting on {frameLabel} — click a frame to change that.</p>
        <input
